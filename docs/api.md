...
> `history_manifest` is always empty and no history is fetched, so a planner
> sees only the current fight. In local practice `deadlines` and
> `observed_tick` are `null`. In `self`/`opponent`, `power_available` is a
> boolean; inside beat records' `before`/`after` states it is `0`/`1`.
