...
class Reader:
    ...
    def market(self, q: dict) -> dict:
        """Sales of the simulated fighter market, newest first (optionally one fighter's)."""
        f = (q.get("fighter") or [None])[0]
        if f is not None and not HEX64.match(f):
            raise ApiError(400, "bad_id", "a fighter ID is 64 lowercase hex digits")
        where, args = ("WHERE fighter_id = ?", (f,)) if f else ("", ())
        total = self.c.execute(f"SELECT COUNT(*) FROM sales {where}", args).fetchone()[0]
        page, per, pages = self._page(q, total)
        items = [json.loads(r[0]) for r in self.c.execute(
            f"SELECT doc FROM sales {where} ORDER BY seq DESC LIMIT ? OFFSET ?", (*args, per, (page - 1) * per))]
        for x in items:
            for k in ("tick", "price", "fee", "bid"):
                if k in x:
                    x[k] = str(x[k])
