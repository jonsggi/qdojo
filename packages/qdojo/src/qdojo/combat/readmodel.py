...
"""
TABLES = ("fighters", "fights", "fight_sides", "contests", "ratings", "cups", "seasons", "ownership")
...
def load_deployment(export_dir: Path | None) -> dict:
    ...
    if export_dir is None:
        return {}
    try:
        return json.loads((Path(export_dir) / "index.json").read_text()).get("deployment") or {}
...
class Syncer:
    ...
    def sync(self, c: CombatContract, deployment: dict, state: dict):
