...
def site(tmp_path_factory):
    ...
    arena = live.Arena(tmp / "arena", lineup, seed=3, deterministic=True, cup_every=600, duel_every=80,
                       market_every=150, log=lambda m: None)
    for _ in range(1300):
        arena.step()
    arena.save()
    out = tmp / "web" / "combat" / "v1"
    export.export_all(arena.w.contract, out, keep=4, deployment=arena.deployment(1.5), extras=arena.extras())
    ...
    yield {"base": f"http://127.0.0.1:{srv.server_address[1]}", "c": arena.w.contract, "out": out, "arena": arena}
    srv.shutdown()
...
def test_market_and_economics(site):
    sales = site["arena"].market.sales
    doc = j(site, "/api/v1/market")
    assert doc["schema"] == "qdojo.combat.api.market.v1" and doc["total"] == len(sales) > 0
    assert [int(x["price"]) for x in doc["items"]] == [x["price"] for x in reversed(sales)][:len(doc["items"])]
    one = sales[0]["fighter_id"]
    assert all(x["fighter_id"] == one for x in j(site, f"/api/v1/market?fighter={one}")["items"])
    assert get(site, "/api/v1/market?fighter=zz")[0] == 400
    econ = j(site, "/api/v1/economics")
    assert econ["schema"] == "qdojo.combat.api.economics.v1" and econ["tiers"]["1"]["stake"] == "5000"
